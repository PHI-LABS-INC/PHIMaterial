...
contract PhiDailyV2Test is PRBTest, StdCheats {
    ...
    function setUp() public {
        ...
        phiDailyV2 =
        new PhiDailyV2(tempTreasury, adminSigner, address(materialObject), address(emissionLogic), tempForwarder, tempGelatoRelay);
        materialObject.setOwner(address(phiDailyV2));
    ...
    function test_Constructor() external {
        assertEq(phiDailyV2.adminSigner(), adminSigner);
    }

    function test_Deployment() public {
        assertEq(phiDailyV2.adminSigner(), adminSigner);
        assertEq(phiDailyV2.emissionLogic(), address(emissionLogic));
        assertEq(phiDailyV2.materialObject(), address(materialObject));
    }
    ...
    function test_claimMaterialObject() public {
        ...
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(test_eventId, 1, coupon, test_expiresIn);

        // Check the claim status
        uint256 count = phiDailyV2.checkClaimCount(testAddress);
        assertEq(count, 1, "The claimed count should be incremented by 1");

        uint256 eachCount = phiDailyV2.checkClaimEachCount(testAddress, 100_003);
        assertEq(eachCount, 1, "The claimed each count should be incremented by 1");

        uint256 status1 = phiDailyV2.checkClaimStatus(testAddress, test_eventId, 1);
        assertEq(status1, 1, "Logic 1: The daily claimed status should be set to _CLAIMED");

        uint256 status2 = phiDailyV2.checkClaimStatus(testAddress, test_eventId, 2);
        assertEq(status2, 0, "Logic 2: The daily claimed status should be set to _CLAIMED");

        uint256 status3 = phiDailyV2.checkClaimStatus(testAddress, test_eventId, 3);
    ...
    function test_claimaterialObjec_withLogic1And2() public {
        ...
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(test_eventId, 1, coupon1, test_expiresIn);
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(test_eventId, 2, coupon2, test_expiresIn);
        vm.stopPrank();
    ...
    function test_batchClaimaterialObject_withLogic1And2() public {
        ...
        phiDailyV2.batchClaimMaterialObject{ value: 0.1 ether }(eventIds, logicIds, coupons, test_expiresIn);
        vm.stopPrank();
        // Check the claim status for both sets of event ID and Logic ID
        for (uint256 i; i < eventIds.length; i++) {
            uint256 isClaimed = phiDailyV2.checkClaimStatus(testAddress, eventIds[i], logicIds[i]);
    ...
    function testFail_AnathorLogicClaim() public {
        ...
        Coupon memory coupon = test_logic1;
        vm.prank(testAddress);
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(test_eventId, 2, coupon, test_expiresIn);
    }
    ...
    function testFail_DoubleClaimMaterialObject() public {
        ...
        vm.prank(testAddress);
        // Event ID and Logic ID are set to 1 for the purpose of this test.
        phiDailyV2.claimMaterialObject(test_eventId, 1, coupon, test_expiresIn);
        // Token ID is set to 1 for the purpose of this test.
        assertEq(phiDailyV2.checkClaimCount(testAddress), 1);
    ...
    function testFail_claimMaterialObject_InvalidCoupon() public {
        ...
        // Event ID and Logic ID are set to 1 for the purpose of this test.
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(1, 1, coupon, 9_999_999_999);
    }
    ...
    function testFail_claimMaterialObject_InvalidEvent() public {
        ...
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(20_230_530, 1, coupon, test_expiresIn);
    }
    ...
    function testFail_claimMaterialObject_InvalidExpiresIn() public {
        ...
        vm.prank(testAddress);
        // Event ID and Logic ID are set to 1 for the purpose of this test.
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(test_eventId, 1, coupon, 999_999_999);
    }
    ...
    function testCheckInitialClaimedSTORGEInitial() public {
        ...
        uint256 eachCount = phiDailyV2.checkClaimEachCount(testAddress, 100_001);
        assertEq(eachCount, 0, "The claimed each count should be 0");

        uint256 status = phiDailyV2.checkClaimStatus(testAddress, 20_230_531, 1);
    ...
    function test_setAdminSigner() public {
        ...
        currentSigner = phiDailyV2.adminSigner();
    ...
    function test_setEmissionLogic() public {
        ...
        address currentLogic = phiDailyV2.emissionLogic();

        vm.prank(admin);
        // Expect the SetEmissionLogic event to be emitted
        vm.expectEmit();
        emit SetEmissionLogic(currentLogic, address(newEmissionLogic));
        phiDailyV2.setEmissionLogic(address(newEmissionLogic));

        currentLogic = phiDailyV2.emissionLogic();
    ...
    function test_setMaterialObject() public {
        ...
        address currentMaterialObject = phiDailyV2.materialObject();

        vm.prank(admin);
        // Expect the SetMaterialObject event to be emitted
        vm.expectEmit();
        emit SetMaterialObject(currentMaterialObject, address(newMaterialObject));
        phiDailyV2.setMaterialObject(address(newMaterialObject));

        currentMaterialObject = phiDailyV2.materialObject();
    ...
    function test_NewEmissionLogic() public {
        ...
        vm.prank(admin);
        phiDailyV2.setEmissionLogic(address(emissionLogic2));
        ...
        uint256 eachCount = phiDailyV2.checkClaimEachCount(testAddress, 100_007);
        assertEq(eachCount, 1, "The claimed each count should be incremented by 1");
    }
    ...
    function test_EmitLogGetMaterialObject() public {
        ...
        vm.expectEmit();
        emit LogGetMaterialObject(address(phiDailyV2), 100_001);
        Coupon memory coupon = test_logic1;
        vm.prank(testAddress);
        // Event ID and Logic ID are set to 1 for the purpose of this test.
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(test_eventId, 1, coupon, test_expiresIn);
    ...
    function test_claimMaterialObject_ExpectRevertSignatureExpired() public {
        ...
        vm.expectRevert(PhiDailyV2.SignatureExpired.selector);
        // Event ID and Logic ID are set to 1 for the purpose of this test.
        phiDailyV2.claimMaterialObject{ value: 0.05 ether }(test_eventId, 1, coupon, test_expiresIn);
    }
    ...
    function test_batchClaimMaterialObject_ExpectRevertDuplicatedCoupons() public {
        ...
        vm.prank(testAddress);
        vm.expectRevert(PhiDailyV2.DuplicatedCoupons.selector);
        phiDailyV2.batchClaimMaterialObject{ value: 0.1 ether }(eventIds, logicIds, coupons, test_expiresIn);
    }
}
